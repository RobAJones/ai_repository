...
// Pin Configuration
#define RESET_BUTTON_PIN 0  // GPIO 0 - Built-in BOOT button on most ESP32 boards
#define WPS_BUTTON_PIN 4    // GPIO 4 - WPS button (can be changed)
#define RESET_HOLD_TIME 3000  // Hold button for 3 seconds to reset
#define WPS_TIMEOUT 120000    // WPS timeout: 2 minutes
...
#define WIFI_RECONNECT_INTERVAL 30000  // Try reconnecting every 30 seconds
#define MAX_RECONNECT_ATTEMPTS 5  // Max attempts before entering config mode
...
<body>
    <div class="container">
        ...
        <div style="margin: 20px 0; text-align: center; color: var(--text-dim);">
            <p>— OR —</p>
        </div>
        
        <button class="btn btn-wps" onclick="startWPS()">
            📶 Use WPS (Push Button)
        </button>
        <p style="margin-top: 10px; font-size: 0.85rem; color: var(--text-dim); text-align: center;">
            Press WPS button on your router, then click above
        </p>
        ...
        function startWPS() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const checkWPS = setInterval(() => {
                        attempts++;
                        
                        if (attempts >= maxAttempts) {
                            clearInterval(checkWPS);
                            showStatus('WPS timeout. Please try again or use manual configuration.', 'error');
                            return;
                        }
                        
                        // Check if connected (you might want to add a status endpoint)
                        showStatus('WPS in progress... (' + (attempts * 5) + 's / 120s)', 'success');
                    }, 5000);
                } else {
            ...
            .catch(error => {
                showStatus('WPS error. Please try again.', 'error');
            });
        }
    </script>
</body>
...
void loop() {
  ...
  if (configMode) {
    ...
    // Check WiFi connection periodically (every 5 seconds)
...
void handleWPSRequest() {
  Serial.println("WPS request received from web interface");
  
  if (wpsRunning) {
    server.send(200, "application/json", "{\"success\":false,\"message\":\"WPS already running\"}");
    return;
  }
...
void checkWiFiConnection() {
  if (WiFi.status() != WL_CONNECTED && !configMode) {
    ...
    // Check signal strength if still technically connected
    if (WiFi.RSSI() < -90) {
      Serial.println("⚠ Weak signal detected: " + String(WiFi.RSSI()) + " dBm");
    }
  } else if (WiFi.status() == WL_CONNECTED) {
    // Optional: Log connection quality
    int rssi = WiFi.RSSI();
    if (rssi < -80) {
...
void attemptReconnect() {
  ...
  if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
    ...
    // Clear the saved config and restart in config mode
    // Uncomment the next line if you want to clear credentials on repeated failures
    // preferences.clear();
  ...
  Serial.println("Time since last attempt: " + String(millis() - lastReconnectAttempt) + "ms");
...
void checkResetButton() {
  ...
  if (currentState && !buttonPressed) {
    ...
    if (holdTime > 1000 && holdTime < 2000 && !resetTriggered) {
      Serial.println("Hold for " + String((RESET_HOLD_TIME - holdTime) / 1000) + " more seconds...");
    } else if (holdTime > 2000 && holdTime < 3000 && !resetTriggered) {
      Serial.println("Hold for " + String((RESET_HOLD_TIME - holdTime) / 1000) + " more second...");
    }
...
// Check WPS button state
void checkWPSButton() {
  // Don't check if WPS already running or in config mode
  if (wpsRunning || configMode) return;
  
  static unsigned long wpsButtonPressTime = 0;
  static bool wpsButtonPressed = false;
  
  bool currentState = (digitalRead(WPS_BUTTON_PIN) == LOW);
  
  if (currentState && !wpsButtonPressed) {
    // Button just pressed
    wpsButtonPressed = true;
    wpsButtonPressTime = millis();
    Serial.println("\n📶 WPS button pressed...");
  } else if (currentState && wpsButtonPressed) {
    // Button still held
    unsigned long holdTime = millis() - wpsButtonPressTime;
    
    if (holdTime >= 1000) {
      // Button held for 1 second - start WPS
    ...
    unsigned long holdTime = millis() - wpsButtonPressTime;
    if (holdTime < 1000) {
      Serial.println("WPS cancelled (button released too soon)");
    }
    wpsButtonPressed = false;
  }
}

// Start WPS connection process
void startWPS() {
  if (wpsRunning) {
  ...
  Serial.println("\n╔════════════════════════════════╗");
  Serial.println("║      WPS CONNECTION MODE       ║");
  Serial.println("╚════════════════════════════════╝");
  Serial.println("");
  Serial.println("📶 WPS (Wi-Fi Protected Setup) Starting...");
  Serial.println("");
  Serial.println("Instructions:");
  Serial.println("1. Press the WPS button on your WiFi router");
  Serial.println("2. Wait for connection (up to 2 minutes)");
  Serial.println("3. ESP32 will auto-configure and connect");
  Serial.println("");
  Serial.println("⏱️  Timeout: 2 minutes");
  Serial.println("");
  ...
  // LED feedback - slow blink during WPS
  pinMode(2, OUTPUT);
  
  // Begin WPS connection
  Serial.println("🔄 Starting WPS handshake...");
  ...
  strcpy(wpsConfig.factory_info.model_name, "ESP32");
  strcpy(wpsConfig.factory_info.device_name, "ESP32-Station");
  
  // Start WPS
  if (esp_wifi_wps_enable(&wpsConfig) == ESP_OK) {
    if (esp_wifi_wps_start(0) == ESP_OK) {
      Serial.println("✓ WPS initiated successfully");
      Serial.println("⏳ Waiting for router WPS button press...");
    } else {
...
void handleWPS() {
  ...
  if (millis() - lastWPSCheck >= 500) {
    ...
    // Print progress dots
    Serial.print(".");
    if ((millis() - wpsStartTime) % 10000 < 500) {
      Serial.println();
      int elapsed = (millis() - wpsStartTime) / 1000;
      int remaining = (WPS_TIMEOUT / 1000) - elapsed;
      Serial.println("⏱️  Time remaining: " + String(remaining) + " seconds");
    }
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    ssid.toCharArray(config.ssid, 32);
    password.toCharArray(config.password, 64);
    config.useStaticIP = false;  // WPS uses DHCP
  ...
  if (millis() - wpsStartTime >= WPS_TIMEOUT) {
    ...
    Serial.println("════════════════════════════════");
    Serial.println("WPS connection failed after 2 minutes");
    Serial.println("");
    Serial.println("Possible reasons:");
    Serial.println("• Router WPS button not pressed");
    Serial.println("• Router WPS disabled");
    Serial.println("• Router out of range");
    Serial.println("• Interference or signal issues");
    Serial.println("");
    Serial.println("Try again or use manual configuration");
    Serial.println("════════════════════════════════");
    ...
    // If no saved config, enter config mode
    if (strlen(config.ssid) == 0) {
