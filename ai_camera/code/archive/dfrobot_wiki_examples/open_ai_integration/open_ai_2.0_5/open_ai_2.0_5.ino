/*
 * ESP32-S3 AI CAM - Complete OpenAI Integration with TTS
 * 
 * Features:
 * - Whisper API for audio transcription
 * - GPT-4 Vision for image analysis
 * - TTS API for speech synthesis of responses
 * - Return Audio list shows TTS responses
 * - Save downloads: analysis text, original audio, TTS audio
 */
...
String base64EncodeFileOptimized(String filepath) {
  ...
  free(inputBuffer);
  if (ret != 0) { free(base64Output); return ""; }
  base64Output[olen] = '\0';
...
// NEW: Generate TTS audio from text
String generateTTSAudio(String text, String analysisID) {
  ...
  doc["model"] = "tts-1";  // or "tts-1-hd" for higher quality
  doc["input"] = text;
  doc["voice"] = "alloy";  // Options: alloy, echo, fable, onyx, nova, shimmer
  doc["response_format"] = "mp3";  // or "opus", "aac", "flac"
  
  String requestBody;
  serializeJson(doc, requestBody);
  
  Serial.printf("Request size: %d bytes\n", requestBody.length());
  Serial.println("Sending to TTS API...");
  ...
  if (httpCode == 200) {
    ...
    // Stream audio data to SD card
    uint8_t buffer[1024];
    size_t totalBytes = 0;
    
    while (https.connected() && (stream->available() > 0 || https.connected())) {
    ...
    ttsFile.close();
    Serial.printf("TTS audio saved: %s (%d bytes)\n", filepath.c_str(), totalBytes);
...
// NEW: Complete OpenAI analysis with TTS
void handleOpenAIAnalyze() {
  Serial.println("\n========================================");
  Serial.println("OpenAI Analysis with TTS Request");
  Serial.println("========================================");
  ...
  if (ttsFile.length() == 0) {
    Serial.println("WARNING: TTS generation failed, continuing without audio");
    ttsFile = "none";
  }
...
// NEW: Download complete analysis package
void handleAnalysisDownload() {
  ...
  String audioQuestion = doc["audioQuestion"].as<String>();
  String audioTTS = doc["audioTTS"].as<String>();
  ...
  if (txtFile) {
    ...
    txtFile.println("Date: " + String(millis()));
    txtFile.println();
  ...
  // Copy files
  // Note: For actual download, files would need to be served via HTTP
  // This creates the package on SD card
  
  server.send(200, "application/json", 
    "{\"success\":true,\"package\":\"" + packageDir + "\"}");
}

// NEW: Serve complete package
void handleAnalysisPackage() {
  String id = server.arg("id");
  String packageDir = "/packages/pkg_" + id;
  
  // For simplicity, serve the text file
  // In production, you'd create a ZIP or serve multiple files
  File txtFile = SD_MMC.open(packageDir + "/analysis.txt", FILE_READ);
  if (!txtFile) {
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n========================================");
  Serial.println("ESP32-S3 AI CAM - Complete with TTS");
  Serial.println("========================================\n");
  ...
  Serial.println("\n========================================");
  Serial.println("READY with TTS! " + WiFi.localIP().toString());
  Serial.println("========================================\n");
}
