...
void setup() {
  ...
  // Test 1: Try connection in insecure mode (skip cert validation)
  // Note: OpenAI's cert chain is complex and ESP32 has trouble with it
  // This is acceptable for testing/development
  Serial.println("STEP 3: Testing HTTPS connection (insecure mode)...");
  Serial.println("Note: Skipping certificate validation due to cert chain issues");
  ...
  bool connected = client.connect("api.openai.com", 443);
  ...
  if (connected) {
    ...
    Serial.println("Error message: " + String(errorBuf));
    Serial.println("→ Network or firewall blocking HTTPS");
    while(1) delay(1000);
  }
  ...
  *(uint32_t*)(testWav + 40) = 160;
  ...
  secureClient.setInsecure();  // Use insecure mode (skip cert validation)
