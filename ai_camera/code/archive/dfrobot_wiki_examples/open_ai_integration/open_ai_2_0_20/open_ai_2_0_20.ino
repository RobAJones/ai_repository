#include "time.h"
/*
 * ESP32-S3 AI CAM - OpenAI Interface v2.0.20
 * 
 * EPS32S3 DEV Module
 * Tools Tab in IDE
 ...
 * - Fixed OpenAI integration by reverting to synchronous processing (working from 2.0.14)
 * - Fixed base64 encoding memory corruption that caused crashes during Vision API calls
 * - Added real-time progress tracking displayed in web interface (no interference with processing)
 */
...
function sendToOpenAI() {
    ...
    fetch('/openai/progress')
      ...
      .then(progress => {
        if (progress.inProgress) {
          ...
          const progressBar = '█'.repeat(Math.floor(progress.percent / 5)) + 
      ...
      .catch(err => {
        // Progress polling failed, but main request continues
      });
  ...
  fetch('/openai/analyze', {
    ...
    .catch(err => {
      clearInterval(progressInterval); // Stop polling
      showStatus('Request failed - check serial monitor', true);
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n\n========================================");
  Serial.println("ESP32-S3 - OpenAI Interface v2.0.20");
  Serial.println("========================================\n");
  ...
  Serial.println("\n========================================");
  Serial.println("SYSTEM READY - Interface v2.0.20");
...
void handleOpenAIAnalyze() {
  ...
  Serial.println("Image: " + imagePath);
  Serial.println("Audio: " + audioPath);
  ...
  if (transcription.startsWith("Error")) {
    ...
    analysisInProgress = false;
    analysisProgressStage = "Error";
    analysisProgressDetail = "Whisper failed: " + transcription;
  ...
  if (analysis.startsWith("Error")) {
    ...
    analysisInProgress = false;
    analysisProgressStage = "Error";
    analysisProgressDetail = "Vision failed: " + analysis;
