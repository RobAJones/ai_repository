/*
 * ESP32-S3 AI CAM - OpenAI with TTS (Non-blocking)
 * 
 * Fixed:
 * - TTS download uses chunked streaming with yield()
 * - Server remains responsive during TTS generation
 * - Added timeout protection
 * - Proper memory management during long operations
 */
...
volatile bool ttsInProgress = false;  // NEW: Track TTS generation

I2SClass i2s_mic;
I2SClass i2s_spk;

// [Same HTML as before - keeping concise]
...
String transcribeAudioWithWhisper(String audioFilePath) {
  ...
  } else transcription = "Error: Whisper API returned " + String(httpCode);
  free(audioData);
  free(requestBody);
  https.end();
...
// FIXED: Non-blocking TTS with yield() calls
String generateTTSAudio(String text, String analysisID) {
  Serial.println("\n=== OpenAI TTS (Non-blocking) ===");
  Serial.println("Text: " + text.substring(0, 50) + "...");
  
  ttsInProgress = true;  // Set flag
  ...
  if (httpCode == 200) {
    Serial.println("TTS response received, streaming to SD...");
    
    WiFiClient* stream = https.getStreamPtr();
    
    ttsCount++;
    ttsFilename = "TTS_" + analysisID + ".mp3";
    String filepath = "/tts/" + ttsFilename;
    
    File ttsFile = SD_MMC.open(filepath, FILE_WRITE);
    if (!ttsFile) {
      Serial.println("ERROR: Cannot create TTS file");
      https.end();
      ttsInProgress = false;
      return "";
    }
    
    // Stream with yield() to prevent watchdog
    uint8_t buffer[512];  // Smaller buffer
    size_t totalBytes = 0;
    ...
    while (https.connected() && (stream->available() > 0 || (millis() - startTime < 30000))) {
      ...
      if (size > 0) {
        ...
        if (len > 0) {
          ...
          if (totalBytes % 10240 == 0) {
            Serial.printf("TTS: %dKB downloaded\n", totalBytes / 1024);
          }
        }
      } else {
        // No data available, yield and wait
        yield();
        delay(10);
      }
      ...
      if (millis() - startTime > 60000) {
        Serial.println("TTS download timeout");
        break;
      }
    }
    
    ttsFile.close();
    Serial.printf("TTS saved: %s (%d bytes)\n", filepath.c_str(), totalBytes);
    
  } else {
    Serial.printf("ERROR: TTS API returned %d\n", httpCode);
  }
...
// HTTP Handlers (same as before - keeping concise)
void handleRoot() { server.send_P(200, "text/html", index_html); }

void handleStream() {
  // Don't serve stream during TTS or recording
...
// [All other handlers same as before - keeping file concise]
void handleCapture() {
  camera_fb_t* fb = esp_camera_fb_get();
...
void handleOpenAIAnalyze() {
  Serial.println("\n========================================");
  Serial.println("OpenAI Analysis with TTS (Non-blocking)");
  Serial.println("========================================");
  ...
  if (ttsFile.length() == 0) {
    Serial.println("WARNING: TTS generation failed");
    ttsFile = "none";
  }
...
void handleAnalysisDownload() {
  ...
  if (txtFile) {
    txtFile.println("=== OPENAI ANALYSIS ===");
    txtFile.println("ID: " + analysisID);
    txtFile.println(text);
    txtFile.close();
  }
  server.send(200, "application/json", "{\"success\":true,\"package\":\"" + packageDir + "\"}");
}

void handleAnalysisPackage() {
  String id = server.arg("id");
  File txtFile = SD_MMC.open("/packages/pkg_" + id + "/analysis.txt", FILE_READ);
  if (!txtFile) { server.send(404, "text/plain", "Package not found"); return; }
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n========================================");
  Serial.println("ESP32-S3 - OpenAI + TTS (Non-blocking)");
  Serial.println("========================================\n");
  ...
  dir = SD_MMC.open("/tts");
  file = dir.openNextFile();
  while (file) { if (!file.isDirectory()) ttsCount++; file = dir.openNextFile(); }
  Serial.printf("✓ %d images, %d audio, %d TTS\n", imageCount, audioCount, ttsCount);
