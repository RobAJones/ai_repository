/*
 * ESP32-S3 AI CAM - OpenAI Integration (Memory Optimized)
 * 
 * Fixed: CORRUPT HEAP error by using streaming base64 encoding
 * and proper memory management for large image files
 */

// [All includes and configuration same as before - keeping concise]
...
// [HTML interface code - same as before, keeping concise for space]
...
// Memory-optimized base64 encoding with streaming
String base64EncodeFileOptimized(String filepath) {
  ...
  Serial.printf("Base64 size will be: %d bytes\n", base64Size);
  
  // Try to allocate base64 buffer in PSRAM first
  char* base64Output = (char*)ps_malloc(base64Size + 1);
  if (!base64Output) {
    Serial.println("PSRAM allocation failed, trying regular malloc");
    base64Output = (char*)malloc(base64Size + 1);
  }
  
  if (!base64Output) {
    Serial.println("ERROR: Failed to allocate base64 output buffer");
    file.close();
    return "";
  }
  
  Serial.println("Encoding in chunks...");
  
  // Process in 3KB chunks (must be multiple of 3)
  const size_t CHUNK_SIZE = 3000;
  uint8_t* inputBuffer = (uint8_t*)malloc(CHUNK_SIZE);
  if (!inputBuffer) {
    free(base64Output);
    file.close();
  ...
  while (file.available()) {
    ...
    int ret = mbedtls_base64_encode(
      (unsigned char*)(base64Output + outputPos), 
      base64Size - outputPos, 
      &olen, 
      inputBuffer, 
      bytesRead
    );
    ...
    if (ret != 0) {
      Serial.printf("ERROR: Base64 encoding failed at chunk, code: %d\n", ret);
      free(inputBuffer);
      free(base64Output);
      file.close();
      return "";
    ...
    if (totalRead % 30000 == 0) {
      Serial.printf("Encoded %d/%d bytes...\n", totalRead, fileSize);
    }
  }
  
  base64Output[outputPos] = '\0';
  
  Serial.printf("Total encoded: %d bytes\n", totalRead);
...
// Compress and resize image before encoding to save memory
String prepareImageForOpenAI(String imageFilePath) {
  Serial.println("\n=== Preparing Image for OpenAI ===");
  
  // Create compressed version for OpenAI
  String compressedPath = "/temp_openai.jpg";
  
  File originalFile = SD_MMC.open(imageFilePath, FILE_READ);
  if (!originalFile) {
    Serial.println("ERROR: Cannot open original image");
    return "";
  }
  ...
  // For images < 100KB, use directly
  if (originalSize < 100000) {
    originalFile.close();
    Serial.println("Image small enough, using original");
    return base64EncodeFileOptimized(imageFilePath);
  }
  
  originalFile.close();
  
  // For larger images, we need to resize using camera
  // Capture a new lower-res image
  Serial.println("Image too large, capturing VGA version for OpenAI...");
  
  sensor_t* s = esp_camera_sensor_get();
  framesize_t original_framesize = s->status.framesize;
  
  // Temporarily set to VGA (640x480)
  s->set_framesize(s, FRAMESIZE_VGA);
  delay(100);
  
  camera_fb_t* fb = esp_camera_fb_get();
  if (!fb) {
    Serial.println("ERROR: Failed to capture VGA frame");
    s->set_framesize(s, original_framesize);
    return "";
  }
  
  // Save compressed version
  File compFile = SD_MMC.open(compressedPath, FILE_WRITE);
  if (!compFile) {
    Serial.println("ERROR: Cannot create temp file");
    esp_camera_fb_return(fb);
    s->set_framesize(s, original_framesize);
    return "";
  }
  
  compFile.write(fb->buf, fb->len);
  compFile.close();
  esp_camera_fb_return(fb);
  ...
  Serial.printf("Compressed size: %d bytes\n", SD_MMC.open(compressedPath, FILE_READ).size());
  
  // Encode the compressed version
  String base64 = base64EncodeFileOptimized(compressedPath);
  
  // Clean up temp file
  SD_MMC.remove(compressedPath);
...
String transcribeAudioWithWhisper(String audioFilePath) {
  ...
  Serial.println("File: " + audioFilePath);
  ...
  // Try PSRAM first for large audio file
  ...
  // Try PSRAM for request body
  uint8_t* requestBody = (uint8_t*)ps_malloc(totalSize);
  if (!requestBody) {
    requestBody = (uint8_t*)malloc(totalSize);
  }
  ...
  Serial.println("Sending to Whisper API...");
  ...
  if (httpCode == 200) {
    ...
    Serial.printf("ERROR: Whisper API returned %d\n", httpCode);
...
String analyzeImageWithGPT4Vision(String imageFilePath, String audioTranscription) {
  ...
  String imageBase64 = prepareImageForOpenAI(imageFilePath);
  if (imageBase64.length() == 0) {
    Serial.println("ERROR: Image preparation failed");
    return "Error: Could not prepare image";
  }
  ...
  // Build JSON request - allocate enough space
  ...
  textPart["type"] = "text";
  String promptText = "Analyze this image concisely. ";
  if (audioTranscription.length() > 0 && !audioTranscription.startsWith("Error")) {
    promptText += "Audio: \"" + audioTranscription + "\". Provide insights on both.";
  }
  ...
  Serial.println("Sending to GPT-4 Vision...");
  
  int httpCode = https.POST(requestBody);
  
  // Clear requestBody to free memory
  requestBody = "";
  ...
  if (httpCode == 200) {
    ...
    String errorResp = https.getString();
    Serial.println("Error: " + errorResp);
...
// [All HTTP handlers - same as before, keeping concise]
...
void handleCapture() {
  ...
  if (file) { file.write(fb->buf, fb->len); file.close(); server.send(200, "application/json", "{\"success\":true,\"filename\":\"IMG_" + String(imageCount) + ".jpg\"}"); }
  else server.send(500, "application/json", "{\"success\":false}");
  esp_camera_fb_return(fb);
}
void handleImageLatest() {
  if (imageCount == 0) { server.send(404, "application/json", "{\"success\":false}"); return; }
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n\n========================================");
  Serial.println("ESP32-S3 AI CAM - OpenAI (Memory Optimized)");
  Serial.println("========================================\n");
  
  SD_MMC.setPins(SD_MMC_CLK, SD_MMC_CMD, SD_MMC_D0);
  if (!SD_MMC.begin("/sdcard", true)) { Serial.println("ERROR: SD Card failed"); return; }
