...
// Speaker pins (if using I2S speaker for TTS output)
#define SPEAKER_BCLK    (GPIO_NUM_10)  // Adjust these if needed
#define SPEAKER_LRCLK   (GPIO_NUM_11)
#define SPEAKER_DIN     (GPIO_NUM_12)
...
void setup() {
  ...
  Serial.println("Version: Camera Rotated + Audio Fixed");
...
void chatInit() {
  ...
  // Configure TTS settings to reduce screeching
  tts.setModel("tts-1");  // Use standard quality (tts-1-hd may cause issues)
  tts.setVoice("alloy");  // Try different voices: alloy, echo, fable, onyx, nova, shimmer
  tts.setSpeed(1.0);      // Normal speed (0.25 to 4.0)
  tts.setFormat("mp3");   // MP3 format
  
  Serial.println("TTS configured: model=tts-1, voice=alloy, speed=1.0");
}
...
String SpeechToText() {
  ...
  // Ensure we're stopping any audio playback before sending
  delay(100);
...
int TextToSpeech(String txt) {
  ...
  // Stop any ongoing I2S operations
  delay(100);
  ...
  if (result == -1) {
    ...
    Serial.println("[TTS] ✓ Audio received and playing");
  }
