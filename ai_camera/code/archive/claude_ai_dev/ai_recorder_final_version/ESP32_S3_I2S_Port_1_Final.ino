/*
 * ESP32-S3 AI CAM - Real-time Audio Recorder
 * 
 * Uses I2S_NUM_1 for audio to avoid conflict with camera
 */
...
// Audio settings - USING I2S_NUM_1 to avoid camera conflict!
#define SAMPLE_RATE       16000
#define I2S_PORT          I2S_NUM_1  // Changed from I2S_NUM_0
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Real-time Recording • I2S Port 1</p>
        </div>
        ...
        <div class="content">
            ...
            <div class="info">
                ⏺️ <strong>Real-time recording:</strong> Audio captures while timer runs. 
                Uses I2S port 1 (camera uses port 0). 10x amplification applied.
            </div>
...
void initI2S_PDM() {
  Serial.println("Initializing I2S_NUM_1 for PDM microphone...");
  ...
  if (err != ESP_OK) {
    Serial.printf("  Failed to install: 0x%x\n", err);
    return;
  }
  
  err = i2s_set_pin(I2S_PORT, &pin_config);
  if (err != ESP_OK) {
    Serial.printf("  Failed to set pins: 0x%x\n", err);
    return;
  }
  
  i2s_zero_dma_buffer(I2S_PORT);
  Serial.printf("  ✓ PDM on I2S_NUM_1 (GPIO %d/%d, %dHz, %dx amp)\n", 
...
void setup() {
  ...
  if (esp_camera_init(&config) == ESP_OK) {
    Serial.println("  ✓ OK (using I2S_NUM_0)");
...
void handleAudioStop() {
  ...
  Serial.printf("Stopped: %s (%.1fs, %d bytes)\n", latestAudio.c_str(), duration, recordedBytes);
  Serial.printf("  Peak: %d → %d after %dx amp\n", peakLevel, min(peakLevel * AMPLIFICATION, 32767), AMPLIFICATION);
