 ...
 * 
 * Records in real-time (no delay after clicking stop)
 * Uses continuous I2S reading instead of blocking recordWAV()
 */
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Real-time PDM Audio Recording</p>
        </div>
        ...
        <div class="content">
            ...
            <div class="info">
                <strong>Real-time recording:</strong> Audio is captured while the timer is running. 
                Click stop anytime to save immediately (10x amplification applied).
            </div>
            ...
            fetch('/audio/start')
                ...
                .then(d => {
                    if (d.success) {
                        ...
                        showStatus('Recording in real-time...');
                    }
                });
        }
        ...
        function stopRecording() {
            ...
            fetch('/audio/stop')
                ...
                .then(d => {
                    ...
                    if (d.success) {
                        showStatus('Saved! Peak: ' + d.peak + ' (original)');
                        loadAudioFiles();
                    }
                });
        }
...
void setup() {
  ...
  Serial.println("\n→ PDM Microphone...");
  initI2S();
  Serial.printf("  ✓ OK (%dx amplification)\n", AMPLIFICATION);
  ...
  if (isRecording && audioFile) {
    ...
    if (result == ESP_OK && bytesRead > 0) {
      ...
      for (int i = 0; i < numSamples; i++) {
        ...
        if (abs(original) > peakLevel) peakLevel = abs(original);
...
void handleAudioStart() {
  ...
  Serial.printf("Recording started (real-time): %s\n", latestAudio.c_str());
  server.send(200, "application/json", "{\"success\":true}");
}
...
void handleAudioStop() {
  ...
  Serial.printf("  Size: %d bytes (%.1f seconds)\n", recordedBytes, (float)recordedBytes / (SAMPLE_RATE * 2));
  Serial.printf("  Original peak: %d (%.1f%% of max)\n", peakLevel, (peakLevel * 100.0) / 32767);
  Serial.printf("  After %dx amp: ~%d (%.1f%%)\n", AMPLIFICATION, 
                min(peakLevel * AMPLIFICATION, 32767), 
                min((peakLevel * AMPLIFICATION * 100.0) / 32767, 100.0));
