/*
 * ESP32-S3 AI CAM - Camera & Audio Recorder (FULLY FIXED)
 * Hardware: DFRobot SKU DFR1154 ESP32-S3 AI CAM
 * 
 * FIXES:
 * - SD card initialization with proper pin configuration
 * - Stop button now works immediately (continuous recording in loop)
 * 
 * Features:
...
// SD Card pins (SD_MMC interface) - CUSTOM PINS FOR DFRobot board
...
void initI2S() {
  ...
  if (err != ESP_OK) {
    ...
    Serial.println("PDM Microphone initialized successfully (GPIO 38/39)");
  }
}
...
void setup() {
  ...
  // Initialize SD card with CUSTOM PINS (critical for DFRobot board!)
...
void loop() {
  ...
  // Handle continuous audio recording - THIS IS THE FIX FOR STOP BUTTON!
  if (isRecording && audioFile) {
    ...
    esp_err_t result = i2s_read(I2S_PORT, buffer, bufferSize * sizeof(int16_t), 
                                 &bytesRead, 0);  // 0 = no wait, allows stop to work
