...
// SD Card SPI pins (adjust if your board uses different pins)
#define SD_CS_PIN       10
...
void setup() {
  ...
  Serial.println("Version: With SD Card Audio Buffering");
  ...
  Serial.println("\n[3/5] Initializing audio input...");
  audioInit();
  Serial.println("✓ Audio input initialized!");
  
  // Test I2S microphone
  Serial.println("\n[4/5] Testing microphone...");
  testI2S();
...
void initSDCard() {
  ...
  if (!SD.begin(SD_CS_PIN, spi)) {
    ...
    Serial.println("⚠️  TTS will play directly (may have audio artifacts)");
...
int TextToSpeechWithSDBuffer(String txt) {
  Serial.println("\n[TTS] Converting text to speech...");
  Serial.printf("[TTS] Text: \"%s\"\n", txt.c_str());
  
  if (!sdCardAvailable) {
    Serial.println("[TTS] ⚠️  No SD card - playing directly (may have artifacts)");
    delay(500);
    int result = tts.message(txt);
    delay(20000); // Your working delay
    return result;
  }
  
  // SD Card is available - use buffering approach
  String audioFile = "/audio/response.mp3";
  
  Serial.println("[TTS] Step 1: Downloading audio from OpenAI to SD card...");
  
  // Delete old file if exists
  if (SD.exists(audioFile.c_str())) {
    SD.remove(audioFile.c_str());
  }
  
  // Request TTS and save to SD card
  // Note: This requires the OpenAI library to support saving to file
  // If your library version doesn't support this, we'll need to use direct HTTP
  File audioFileHandle = SD.open(audioFile.c_str(), FILE_WRITE);
  if (!audioFileHandle) {
    Serial.println("[TTS] ✗ Failed to create audio file on SD card");
    return -1;
  }
  ...
  // Get audio (this will play it, but we want to save it)
  // For now, we'll use the direct playback with longer delay
  audioFileHandle.close();
  
  Serial.println("[TTS] ⚠️  Library limitation: Cannot save TTS to SD card directly");
  Serial.println("[TTS] Using direct playback with extended delay...");
  
  int result = tts.message(txt);
  
  if (result == -1) {
    Serial.println("[TTS] ✗ Failed to get audio");
    return -1;
  }
  
  Serial.println("[TTS] ✓ Audio playing...");
  
  // Extended delay for smooth playback (your working value)
  delay(20000);
...
void stopRecording() {
  ...
  if (txt != NULL && txt.length() > 0) {
    ...
    if (response != NULL && response.length() > 0) {
      ...
      if (TextToSpeechWithSDBuffer(response) == -1) {
