...
void setup() {
  ...
  Serial.println("    No SD Card - Direct Audio");
  ...
  if (I2S_SPEAKER.begin(I2S_MODE_STD, 24000, I2S_DATA_BIT_WIDTH_16BIT, I2S_SLOT_MODE_MONO)) {
    Serial.println("OK");
  } else {
    Serial.println("FAIL");
  }
...
void setupWeb() {
  server.on("/", HTTP_GET, []() {
    server.send(200, "text/html",
      ...
      "<img id='c' alt='Camera'/>"
      "<p style='color:#888'>Press BOOT to record<br>Direct audio streaming (no SD card)</p>"
      "</body></html>"
    );
  });
...
bool streamTTS(String txt) {
  ...
  digitalWrite(MAX_MODE, HIGH);  // Enable speaker
