...
void loop() {
  // Only handle web requests when not processing
  if (!isProcessing) {
    server.handleClient();
  }
...
void setupWebServer() {
  server.on("/", HTTP_GET, []() {
    ...
    String html = "<!DOCTYPE html><html><head><title>DFRobot AI Camera</title>"
                  ...
                  "  fetch('/status',{timeout:2000})"
                  "  .then(r=>r.json())"
                  "  .then(d=>{"
                  "    processingMode = d.processing;"
                  "    let cls='status';"
                  ...
                  "  });"
                  "}"
                  "setInterval(updateStatus, 1000);"  // Increased from 500ms to 1000ms
                  "updateStatus();"
                  "</script>"
                  "</head><body>"
                  "<div class='container'>"
...
String imageAnswering(String txt) {
  ...
  yield();  // Feed watchdog after API call
  return response;
}
...
bool downloadTTSToSD(String text, String filename) {
  ...
  while (http.connected()) {
    ...
    if (available > 0) {
      ...
      delay(1);
      yield();  // Feed watchdog during download
    }
