/*
 * ESP32-S3 AI CAM - Audio Recorder (NOISE FIX)
 * 
 * Recording works but audio is noisy - trying different configurations
 * to fix audio quality
 */
...
// Audio configuration - Try lower sample rate first
#define SAMPLE_RATE       16000  // Lower rate for clearer audio
...
void initI2S() {
  ...
  // Configuration optimized for clean audio
  i2s_config_t i2s_config = {
    .mode = (i2s_mode_t)(I2S_MODE_MASTER | I2S_MODE_RX),
    .sample_rate = SAMPLE_RATE,
    .bits_per_sample = I2S_BITS_PER_SAMPLE_32BIT,  // Try 32-bit for better quality
    .channel_format = I2S_CHANNEL_FMT_ONLY_LEFT,   // Try LEFT channel
  ...
  if (err != ESP_OK) {
    Serial.printf("I2S pin set failed: 0x%x\n", err);
    return;
  }
  
  i2s_zero_dma_buffer(I2S_PORT);
  Serial.printf("✓ I2S initialized: %dHz, 32-bit, LEFT channel\n", SAMPLE_RATE);
}
...
void loop() {
  ...
  if (isRecording && audioFile) {
    ...
    int32_t buffer32[bufferSize];  // Read as 32-bit
    int16_t buffer16[bufferSize];  // Convert to 16-bit
    size_t bytesRead = 0;
    
    esp_err_t result = i2s_read(I2S_PORT, buffer32, bufferSize * sizeof(int32_t), 
    ...
    if (result == ESP_OK && bytesRead > 0) {
      ...
      for (int i = 0; i < samples; i++) {
        buffer16[i] = (buffer32[i] >> 16) & 0xFFFF;
      }
      
      // Write 16-bit samples to file
      audioFile.write((uint8_t*)buffer16, samples * sizeof(int16_t));
...
void handleAudioStop() {
  ...
  Serial.printf("Stopped: %d bytes (%.1f seconds at %dHz)\n", 
                recordedBytes, (float)recordedBytes / (SAMPLE_RATE * 2), SAMPLE_RATE);
