/*
 * Credentials Manager for ESP32-S3 AI Camera v3.1.1
 * 
 * v3.1.1 PATCH:
    ...
    setupServer->on("/configure", HTTP_POST, [&apSSID]() {
      ...
      if (ssid.length() > 0 && apiKey.length() > 0) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          response += "1. Save this IP address<br>";
          response += "2. Device will restart in 5 seconds<br>";
          response += "3. Reconnect to your WiFi network<br>";
          response += "4. Open <strong>http://" + assignedIP + "</strong> in your browser";
          response += "</div>";
          response += "</div>";
          response += "<script>";
          ...
          delay(5000);  // Give user time to see and copy IP
          
          configComplete = true;
          WiFi.softAP(apSSID.c_str()); // Restart AP to cleanly disconnect clients
        } else {
...
<body>
    <div class="container">
        <h1>🤖 AI Camera Setup<span class="version-badge">v3.1.1</span></h1>
